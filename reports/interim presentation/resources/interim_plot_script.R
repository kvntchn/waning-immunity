...
plt32 <- ggplot(aes(x = time), data = trajectory.ode) +
  ...
  geom_path(aes(y = incidence, col = 'Fix omega'), alpha = 0.7, lwd = 1.3,
  ...
  geom_path(aes(y = incidence, col = 'Decrease omega \nby 50%'), alpha = 0.9, lwd = 0.8,
  ...
  geom_path(aes(y = incidence, col = 'Increase omega \nby 50%'), alpha = 0.9, lwd = 0.7,
  ...
  scale_colour_manual(name = "Model",
                      ...
                      breaks = c('Initial Model', 'Fix omega', 'Decrease omega \nby 50%', 'Increase omega \nby 50%')) + 
  theme_bw()
